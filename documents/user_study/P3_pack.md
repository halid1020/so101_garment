# User-study session pack — participant P3
...
pack afterwards. Condition order for P3 (balanced Latin square):
**C3 -> C4 -> C2 -> C1**, presented to the participant as A -> B -> C -> D.
...
## Condition A (experimenter key: C3 — armplane pipeline, `telegrip` split IK, OOE `project`)
...
## Condition B (experimenter key: C4 — native upstream Telegrip stack (own UI + IK, no envelope))
...
## Condition C (experimenter key: C2 — armplane pipeline, `pink_relaxed` tracker, OOE `project`)
...
## Condition D (experimenter key: C1 — armplane pipeline, `pink_full` tracker, OOE `project`)
