# User-study session pack — participant P2
...
pack afterwards. Condition order for P2 (balanced Latin square):
**C2 -> C3 -> C1 -> C4**, presented to the participant as A -> B -> C -> D.
...
## Condition A (experimenter key: C2 — armplane pipeline, `pink_relaxed` tracker, OOE `project`)
...
## Condition B (experimenter key: C3 — armplane pipeline, `telegrip` split IK, OOE `project`)
...
## Condition C (experimenter key: C1 — armplane pipeline, `pink_full` tracker, OOE `project`)
...
## Condition D (experimenter key: C4 — native upstream Telegrip stack (own UI + IK, no envelope))
