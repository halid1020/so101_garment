# User-study session pack — participant P1
...
pack afterwards. Condition order for P1 (balanced Latin square):
**C1 -> C2 -> C4 -> C3**, presented to the participant as A -> B -> C -> D.
...
## Condition A (experimenter key: C1 — armplane pipeline, `pink_full` tracker, OOE `project`)
...
## Condition B (experimenter key: C2 — armplane pipeline, `pink_relaxed` tracker, OOE `project`)
...
## Condition C (experimenter key: C4 — native upstream Telegrip stack (own UI + IK, no envelope))
...
## Condition D (experimenter key: C3 — armplane pipeline, `telegrip` split IK, OOE `project`)
