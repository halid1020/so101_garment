...
against the session's target --- so an operator drives a whole session from
the window rather than the terminal. \unjustified{} the half-frame target
...
overhead depth image, the twelve-dimensional proprioceptive state (five
joints and a gripper per arm), and the executed target
(Section~\ref{sec:contract}). Depth is sixteen-bit and is stored losslessly
outside the colour-video pipeline, because a depth image is not
