...
its configured resolution. Each camera is likewise bound to a stable
per-socket alias rather than an enumeration index, so replugging a hub can
never silently swap one view for another between collection and deployment;
the same alias drives both the live monitor and the recorder. A preflight
check confirms these conditions and
