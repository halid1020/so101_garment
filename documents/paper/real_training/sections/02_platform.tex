...
and the head-mounted controller uses a direct link. The volume the dataset is
written to is a peripheral too, and on this rig it hangs off one of those same
hubs --- an arrangement worth knowing about, since it makes the storage share the
fate of the sensors (Sections~\ref{sec:replicability} and~\ref{sec:difficulties}).
Shared-bus bandwidth is
...
That constraint is a ceiling rather than a tax, and with the fingertip cameras
attached the full sensor suite exceeds it: each controller admits only a few
compressed streams at the resolution and rate we record, and the rig carries
more cameras than its two controllers can admit together. The consequence is
not a slower stream but a missing one, which Section~\ref{sec:difficulties}
takes up. Because the ceiling is a property of the wiring rather than of any
one session, the readiness check counts a stream selection against the
controllers it is spread across and reports the shortfall before a session
opens a single camera; recording a subset that fits is then a decision the
operator makes deliberately, rather than a camera that fails for them.
