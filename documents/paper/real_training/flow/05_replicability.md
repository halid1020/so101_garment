...
P6  Readiness procedure: a preflight check confirms the rig is at the fixed
    collection state — calibrated arms on stable aliases, arm buses that can
    actually be OPENED (they re-enumerate under restricted ownership on any hub
    reset, so an operator outside the privileged group loses an arm without
    touching anything), protected cables, home poses, opened cameras and depth
    device, disk headroom, and the host-tuning items that reduce timing jitter.
