...
handle axes; dedicated buttons enable the arms (torque on, ready pose)
and park them (rest pose, torque off), one button brackets recorded
demonstration episodes around a homing move, and another re-homes the
arms without interrupting a recording; triggers drive the grippers. An
optional recording layer captures each demonstration as an episode in a
standard imitation-learning dataset format at a fixed frame rate, and
writes alongside it a full-rate log of every control signal (joint
states and commands, end-effector poses, and the raw operator inputs)
for offline analysis; an episode interrupted by a fault is discarded
automatically and the arms return to rest. The trigger maps to a \emph{capped} jaw opening~---
...
\paragraph{Leader-arm interface.}
An alternative input replaces the headset with two passive replicas of
the follower arms. The operator back-drives these leaders by hand, and
each follower joint tracks its counterpart on the corresponding leader
directly, without inverse kinematics~--- the two arms share a joint
convention, so a leader joint angle is already the follower's target. A
keyboard replaces the controller buttons. There is no clutch: the
followers mirror the leaders continuously once enabled, and a per-joint
velocity limit is the only smoothing, chiefly to soften the initial
catch-up. The leader jaw drives the same capped gripper command as the
trigger, so demonstrations collected through either interface share one
format and one control convention.

That velocity limit must be expressed against elapsed time rather than against
the control period. The distinction sounds pedantic and is not: a limit applied
once per iteration silently becomes a limit per iteration, so whenever the
control loop is delayed --- and it is delayed exactly when it matters, while the
same host is capturing and compressing several camera streams --- the followers
are held to a fraction of the intended speed. The operator sees the followers
trail the leaders, worst during recording, which is the one situation where the
demonstration is being kept. Deriving each step from the measured interval keeps
the limit a true angular velocity at any loop rate, and capping how much elapsed
time a single step may claim stops a long delay from being spent as one
discontinuous jump.

The same lag reaches the operator by a second route, from the bus rather than
the clock. A serial bus to the arms drops a reply occasionally; this is ordinary
and not worth preventing, so what matters is the controller's answer to it.
Reading again within the same tick costs a fraction of the period and usually
succeeds. Abandoning the tick, which is the obvious alternative, costs the whole
period and leaves the followers holding the previous target for its duration ---
the very quantity the velocity limit exists to bound, arriving by a different
path. An asymmetry hid this from us for some time: the reads to the followers
already asked the bus to retry and the read from the leaders did not, so
identical behaviour on two identical buses was visible on one side only, and
presented itself as a worse leader bus rather than as a difference in how the
two reads were made.
