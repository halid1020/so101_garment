...
`paper/teleoperation/` §"Bimanual user study (protocol)". The paper fixes
...
| C4 | upstream Telegrip stack (own UI + IK, no envelope) | `venv/bin/python tool/telegrip_native.py --autoconnect` (see `markdowns/telegrip_native.md`) |
...
1. **Setup (before participant arrives):**
   - `source setup.sh`; confirm arms respond (`tool/identify_arms.py`
...
episode for post-hoc EE smoothness analysis with `sim_benchmark/metrics.py`.
...
- Write results into `paper/teleoperation/sections/05_experiments.tex`
