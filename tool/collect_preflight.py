...
def run_checks(hardware: bool) -> list[CheckResult]:
    ...
    for fn in file_checks:
        try:
            ...
            results.append(CheckResult(fn.__name__, FAIL, f"check error: {e}"))
    results.append(
        CheckResult(
            "wrist-roll limits",
            WARN,
...
def main() -> int:
    ...
    results = run_checks(hardware=not args.no_hardware)
