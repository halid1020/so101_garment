...
class SO101DualArm:
    ...
    def _setup_motors(self, calibration: MotorCalibration, port_id: str) -> FeetechMotorsBus:
    ...
    def move_to_joint_pose(self, desired_pos_0: Dict[str, float], desired_pos_1: Dict[str, float], duration: float) -> None:
        """
    ...
    def move_to_norm_pose(self, norm_pos_0: Dict[str, float], norm_pos_1: Dict[str, float], duration: float) -> None:
        """
        ...
        """
        if not self.limits:
            raise ValueError("Cannot move to normalized pose: 'joint_limits_deg' missing from robot config.")
        ...
        def map_norm_to_deg(norm_dict: Dict[str, float]) -> Dict[str, float]:
            ...
            for joint, norm_val in norm_dict.items():
                ...
                deg_dict[joint] = min_val + ((norm_val + 1.0) / 2.0) * (max_val - min_val)
            return deg_dict
    ...
    def move_to_ee_pose(self, ee_pose_0: Dict[str, float], ee_pose_1: Dict[str, float], duration: float, ik_solver: Any) -> None:
        """
        ...
        if ik_solver is None:
            raise NotImplementedError("An external IK solver is required to map Cartesian EE poses to joint angles.")
